...
export default function ModalCarpetasPDFs({ show, onHide, jugadorId }) {
  ...
  const fetchCarpetas = () => {
    fetch(`http://localhost:8000/api/carpetas/?jugador_id=${jugadorId}`, {
  ...
  const fetchPDFs = (carpetaId) => {
    fetch(`http://localhost:8000/api/pdfs/?carpeta=${carpetaId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then((data) =>
  ...
  const crearCarpeta = () => {
    ...
    fetch("http://localhost:8000/api/carpetas/", {
      method: "POST",
      headers: {
  ...
  const subirPDF = async (carpetaId, file) => {
    ...
    setSubiendo(true);
    await fetch("http://localhost:8000/api/pdfs/", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: formData,
    });
