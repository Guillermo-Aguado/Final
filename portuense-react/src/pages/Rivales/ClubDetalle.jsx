...
export default function ClubDetalle() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/api/clubes-rivales/${id}/`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then(setClub);

    fetch(`http://localhost:8000/api/comentarios-club-rival/?club=${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then(setComentarios);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("http://localhost:8000/api/comentarios-club-rival/", {
      method: "POST",
      headers: {
  ...
  const eliminarComentario = async (comentarioId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/api/comentarios-club-rival/${comentarioId}/`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
