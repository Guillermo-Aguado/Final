...
export default function DetalleJugadorRival() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/api/comentarios-rivales/jugador/${jugadorId}/`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
  ...
  const eliminarComentario = (comentarioId) => {
    if (window.confirm("¿Eliminar este comentario?")) {
      fetch(`http://localhost:8000/api/comentarios-rivales/${comentarioId}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
        .then((res) => {
  ...
  const eliminarJugador = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/jugadores-rivales/${jugadorId}/`, {
        method: "DELETE",
        headers: {
  ...
  const handleSubmitComentario = async (e) => {
    ...
    const res = await fetch(`http://localhost:8000/api/comentarios-rivales/`, {
      method: "POST",
      headers: {
